...
def substitute_args_into_func_display_name(
    func_name: str, param_values: dict[str, Any]
) -> str:
    result = func_name
    ...
    def wrapper(*args: P.args, **kwargs: P.kwargs) -> StepWrapperResult:
        ...
        try:
            ...
            return StepWrapperResult(
                func_name=func_name, get_param_values=get_param_values, result="passed"
            )
...
class Step:
    ...
    def __str__(self):
        func_name_with_vars = substitute_args_into_func_display_name(
            self.step_wrapper_result.func_name,
            self.step_wrapper_result.get_param_values(),
        )
...
class ScenarioRunner:
    ...
    def append_step(
        self, step_wrapper_result: StepWrapperResult, name: StepName
    ) -> "ScenarioRunner":
        self.steps.append(Step(step_wrapper_result=step_wrapper_result, name=name))
        return self
    ...
    def run_all_steps(self):
        ...
        if self.failures:
            msgs = [
                f"Step {failure.name} failed: {failure.error_message} stack trace: {failure.error_stack_trace}"
...
class LogAssertions:
    ...
    def exists(self):
        matching_logs = [
            ...
            and (
                not hasattr(self, "_extra_vars")
                or all(log["extra"].get(k) == v for k, v in self._extra_vars.items())
            )
        ]
...
class ObjectBuilder(Generic[T]):
    ...
    def create(self) -> T:
        ...
        return strategies.builds(
            self._cls, **{k: strategies.just(v) for k, v in self._overrides.items()}
        ).example()
...
strategies.lists = lambda elements, **kwargs: original_lists(
    elements, min_size=kwargs.get("min_size", 1), **kwargs
)
