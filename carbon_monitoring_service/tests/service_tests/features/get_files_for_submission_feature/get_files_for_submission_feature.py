...
def test_when_submission_is_not_found(
    ...
    ctx.runner.given(
        icos_api_is_configured_with_submission_OBJECT_ID_to_submission_not_found(
            ctx.submission_id, ctx.requests_mock
        )
    ).and_also(a_monitored_site_is_added(ctx)).then(
        lambda_should_throw(icos.SubmissionNotFoundException, ctx)
    ).and_also(
        there_should_be_a_log_with_severity_LEVEL_and_message_MESSAGE(
...
def test_when_submission_is_not_valid_sha256(
    ...
    ctx.runner.given(
        ...
        lambda_should_throw(icos.SubmissionMalformedException, ctx)
    ).and_also(
        there_should_be_a_log_with_severity_LEVEL_and_message_MESSAGE(
            f"invalid submission id: {ctx.submission_id}", "ERROR", ctx.log_capture
        )
    ).run_all_steps()
...
def test_when_site_is_not_found(
    ...
    ctx.runner.given(
        icos_api_is_configured_with_submission_OBJECT_ID_to_return_files_FILE_URLS_for_submission(
            ctx.file_urls, ctx.submission_id, ctx.requests_mock
        )
    ).then(lambda_should_throw(SiteNotFoundException, ctx)).and_also(
        there_should_be_a_log_with_severity_LEVEL_and_message_MESSAGE(
            f"handler failed: site {ctx.site} not found", "ERROR", ctx.log_capture
        )
    ).run_all_steps()
...
def test_when_files_are_fetched_for_site(
    ...
    ).and_also(a_monitored_site_is_added(ctx)).when(lambda_is_invoked(ctx)).then(
        result_should_equal_file_url_and_site(ctx)
    ).and_also(the_monitored_sites_last_fetched_is_updated(ctx)).and_also(
        there_should_be_a_log_with_severity_LEVEL_and_message_MESSAGE(
            "handler started", "INFO", ctx.log_capture
        )
    ).and_also(
        there_should_be_a_log_with_severity_LEVEL_and_message_MESSAGE(
            "handler completed", "INFO", ctx.log_capture
        )
    ).run_all_steps()
