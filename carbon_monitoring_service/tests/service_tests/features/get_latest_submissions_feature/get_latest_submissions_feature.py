...
def test_when_no_submissions_are_available_for_site(
    ...
    ctx.runner \
        ...
        .and_also(there_should_be_a_log_with_severity_LEVEL_and_message_MESSAGE("no submissions found", "ERROR", ctx.log_capture)) \
        .run_all_steps()
...
def test_when_submission_url_is_malformed(
    ...
    ctx.runner \
        .given(a_monitored_site_is_added_with_last_fetched_LAST_FETCHED(ctx)) \
        .and_also(icos_api_is_configured_to_return_invalid_submission_url_with_submission_time_SUBMISSION_TIME_and_monitored_site_MONITORED_SITE(submission_time, ctx.monitored_sites[0].name, ctx.requests_mock)) \
        .then(lambda_should_throw_error(ctx, SubmissionObjectIdMalformed)) \
        .and_also(there_should_be_a_log_with_severity_LEVEL_and_message_MESSAGE_and_extras_EXTRAS("handler failed: submission object id malformed: invalid_unparsable", "ERROR", ctx.scoped_log_vars, ctx.log_capture)) \
        .run_all_steps()
...
def test_when_a_new_submission_is_added_for_site(
    ...
    ctx.runner \
        ...
        .and_also(there_should_be_a_log_with_severity_LEVEL_and_message_MESSAGE_and_extras_EXTRAS("handler started", "INFO", ctx.scoped_log_vars, ctx.log_capture)) \
        .and_also(there_should_be_a_log_with_severity_LEVEL_and_message_MESSAGE_and_extras_EXTRAS("handler completed", "INFO", ctx.scoped_log_vars, ctx.log_capture)) \
        .run_all_steps()
